...
p1 <- ggplot() +
  ...
  geom_text_repel(data=happiness %>%
              top_n(-5,row_number(rank_diff)),
            aes(x=0.97, y=rank_2015, label=country),
            hjust=1, size=10, direction='y', force=0.1, family='roboto', color=top5col[2]
            ) +
  geom_text_repel(data=happiness %>%
              top_n(-5,row_number(rank_diff)),
            aes(x=3.03, y=rank_2019, label=country),
  ...
  geom_text(aes(x=1, y=-3, label='2015'), hjust=1, size=25, family='roboto', color='#a8c4cc') +
  geom_text(aes(x=3, y=-3, label='2019'), hjust=0, size=25, family='roboto', color='#a8c4cc') +
  ...
  theme(
    ...
    plot.caption = element_text(family='roboto', hjust = 0.5, vjust = 0.5, size=28, color="darkgray")
  )
